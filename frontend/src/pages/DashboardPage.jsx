...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      setLoadingData(true);
      setError('');
      const [summaryRes, balancesRes, rentRes] = await Promise.all([
  ...
  return (
    <div className="container">
      ...
      {/* Net Balance Card */}
      <div className="card" style={{ borderLeft: '4px solid var(--accent-blue)', padding: '20px' }}>
        <div style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-secondary)', marginBottom: '4px' }}>
          My Net Balance
        </div>
        <div style={{ fontSize: '32px', fontWeight: 800, color: myBalance >= 0 ? 'var(--accent-green)' : 'var(--accent-red)', marginBottom: '16px' }}>
          {myBalance >= 0 ? '+' : ''}{formatCurrency(myBalance)}
        </div>
        
        <div className="grid-2">
          <div style={{ padding: '10px', background: 'var(--bg-secondary)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
          ...
          <div style={{ padding: '10px', background: 'var(--bg-secondary)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
            ...
            <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--accent-red)', marginTop: '2px' }}>
              {formatCurrency(iOwe)}
            </div>
          </div>
        </div>
